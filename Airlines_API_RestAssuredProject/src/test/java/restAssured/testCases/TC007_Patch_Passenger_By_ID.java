...
public class TC007_Patch_Passenger_By_ID extends BaseClass
{
	@BeforeClass
	public void updatePassengerPartially() 
	{
		String updatePassName= read.updated2_passenger_name();
		String PassId= read.get_passenger_get_id();
		
		logger.info("*********Started TC007_Patch_Passenger_By_ID**********");
		RestAssured.baseURI= "https://api.instantwebtools.net/v1";
		httprequest = RestAssured.given();
		JSONObject requestParams = new JSONObject();
		
		requestParams.put("name", updatePassName);
		
		httprequest.header("Content-Type", "application/json");
		httprequest.body(requestParams.toJSONString());
		
		response = httprequest.request(Method.PATCH, "/passenger/"+PassId);
	}
	...
	public void checkResposeBody()
	{
		...
		Assert.assertEquals(jsonPathEvaluator.get("message"), "Passenger data updated successfully.");
	}
	...
	{
		logger.info("*********Finished TC007_Patch_Passenger_By_ID**********");
	}
