...
public class TC006_Put_Passenger_By_ID extends BaseClass
{
	@BeforeClass
	public void updatePassenger()
	{
		String updatedPassName= read.updated_passenger_name();
		String updatedTrip= read.updated_passenger_trips();
		String updatedAirlineId= read.updated_passenger_airline();
		String PassId= read.get_passenger_get_id();
		
		logger.info("*********Started TC006_Put_Passenger_By_ID**********");
		RestAssured.baseURI= "https://api.instantwebtools.net/v1";
		httprequest = RestAssured.given();
		JSONObject requestParams = new JSONObject();
		
		requestParams.put("name",updatedPassName);
		requestParams.put("trips", updatedTrip);
		requestParams.put("airline", updatedAirlineId);
		
		httprequest.header("Content-Type", "application/json");
		httprequest.body(requestParams.toJSONString());
		
		response = httprequest.request(Method.PUT, "/passenger/"+PassId);
	}
	...
	public void checkResposeBody()
	{
		...
		Assert.assertEquals(jsonPathEvaluator.get("message"), "Passenger data put successfully completed.");
	}
	...
	{
		logger.info("*********Finished TC006_Put_Passenger_By_ID**********");
	}
